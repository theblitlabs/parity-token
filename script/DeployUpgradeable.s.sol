...
contract DeployUpgradeable is Script {
    ...
    function _saveAddresses(address proxy, address implementation) internal {
        ...
        proxyCmd[0] = "bash";
        proxyCmd[1] = "-c";
        proxyCmd[2] = string.concat(
            "sed -i '' 's/^PROXY_ADDRESS=.*$/PROXY_ADDRESS=",
            vm.toString(proxy),
            "/' .env"
        );
        ...
        tokenCmd[0] = "bash";
        tokenCmd[1] = "-c";
        tokenCmd[2] = string.concat(
            "sed -i '' 's/^TOKEN_ADDRESS=.*$/TOKEN_ADDRESS=",
            vm.toString(proxy),
            "/' .env"
        );
        ...
        implCmd[2] = string.concat(
            "sed -i '' 's/^IMPLEMENTATION_ADDRESS=.*$/IMPLEMENTATION_ADDRESS=",
        ...
        try vm.ffi(proxyCmd) {
            try vm.ffi(tokenCmd) {
                try vm.ffi(implCmd) {
                    ...
                    console2.log(
                        "Warning: Could not save implementation address to .env"
                    );
                }
            } catch {
